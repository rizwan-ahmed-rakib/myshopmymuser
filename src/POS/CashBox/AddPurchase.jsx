...
//       try {
//         const res = await axios.get(`${BASE_URL}/purchase/suppliers/`);
//         setSuppliers(res.data);
//       } catch (err) {
//         console.error("Error fetching suppliers:", err);
//         setError("সাপ্লায়ার লোড করতে সমস্যা হয়েছে।");
//       } finally {
...
//       try {
//         const res = await axios.get(`${BASE_URL}/inventory/products/`);
...
//
//   const addToCart = () => {
//     if (!newProduct.product || !newProduct.quantity || !newProduct.cost) return;
//
//     // Check if already in cart — update quantity instead of duplicate
//     const existing = cartItems.find(item => item.productId === newProduct.productId);
//     if (existing) {
//       setCartItems(cartItems.map(item =>
//         item.productId === newProduct.productId
//           ? {
//               ...item,
//               quantity: item.quantity + parseInt(newProduct.quantity),
//               total: (item.quantity + parseInt(newProduct.quantity)) * parseFloat(newProduct.cost)
//             }
//           : item
//       ));
//     } else {
//       const total = parseInt(newProduct.quantity) * parseFloat(newProduct.cost);
//       setCartItems([...cartItems, {
//         id: Date.now(),
...
//     }
//
//     setNewProduct({ product: '', productId: '', quantity: 1, cost: '' });
//   };
//
//   const removeFromCart = (id) => {
//     setCartItems(cartItems.filter(item => item.id !== id));
...
//
//     if (!purchaseData.supplier || cartItems.length === 0) {
//       setError('সাপ্লায়ার সিলেক্ট করুন এবং কমপক্ষে একটি পণ্য যোগ করুন।');
//       return;
//     }
//
//     setSubmitting(true);
...
//       };
//
//       await axios.post(`${BASE_URL}/purchase/purchases/`, payload);
//
//       setSuccess('Purchase order সফলভাবে তৈরি হয়েছে!');
//       setPurchaseData({
//         supplier: '',
//         date: new Date().toISOString().split('T')[0],
//         paymentStatus: 'Due',
...
//         setError(messages);
//       } else {
//         setError("Purchase order তৈরি করতে সমস্যা হয়েছে।");
//       }
//     } finally {
//       setSubmitting(false);
//     }
...
//                 >
//                   <option value="">{loadingSuppliers ? 'লোড হচ্ছে...' : 'Select Supplier'}</option>
//                   {suppliers.map(supplier => (
...
//                       cost: selected ? selected.cost_price ?? selected.cost ?? '' : ''
//                     });
//                   }}
//                   disabled={loadingProducts}
//                 >
//                   <option value="">{loadingProducts ? 'লোড হচ্ছে...' : 'Select Product'}</option>
//                   {products.map(product => (
//                     <option key={product.id} value={product.name}>{product.name}</option>
//                   ))}
//                 </select>
...
//             <button
//               onClick={addToCart}
//               disabled={!newProduct.product || !newProduct.quantity || !newProduct.cost}
...
//               onClick={handleSubmit}
//               disabled={submitting}
//               className="w-full mt-6 bg-green-500 text-white py-3 rounded-lg font-semibold hover:bg-green-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
//             >
//               {submitting ? (

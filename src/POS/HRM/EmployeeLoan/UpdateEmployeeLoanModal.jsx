...
import {
  FaMoneyBillWave, FaMobileAlt, FaUniversity,
  FaRegStickyNote, FaHashtag, FaInfoCircle,
  FaCoins, FaCalendarAlt, FaHistory, FaCogs
} from "react-icons/fa";
...
const UpdateEmployeeLoanModal = ({ isOpen, onClose, onSuccess, advanceData }) => {
    ...
    // মোডাল ওপেন হলে এবং ডেটা এভেইলেবল থাকলে ফর্ম ফিল্ড পপুলেট করা[cite: 11]
    useEffect(() => {
        if (advanceData && isOpen) {
            setForm({
                amount: advanceData.amount || "",
    ...
    // পেমেন্ট মেথড অটো-আপডেট লজিক[cite: 11]
    useEffect(() => {
        const counts = [Number(form.paid_cash) > 0, Number(form.paid_mobile) > 0, Number(form.paid_bank) > 0].filter(Boolean).length;
        let method = "cash";
        if (counts > 1) method = "hybrid";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await api.patch(`/api/users/employee-loans/${advanceData.id}/`,
                {
                    ...form,
        ...
        >
            <div className="space-y-6">
                ...
                <div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100 flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Editing Loan For</p>
                        <h3 className="font-bold text-gray-900">{advanceData?.user_name}</h3>
                    </div>
                </div>

                {/* লোন প্রিন্সিপাল এবং ইনস্টলমেন্ট গ্রিড */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                ...
                <div className="space-y-2">
                    ...
                    <textarea
                        ...
                        rows="2"
                        placeholder="Reason for loan adjustment..."
                    />
                </div>
                    ...
                    {Number(form.paid_bank) > 0 && (
                        <div className="p-6 bg-blue-50/50 rounded-3xl border border-blue-100 animate-in slide-in-from-top-4 duration-500 space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <input name="bank_name" value={form.bank_name} onChange={handleChange} className="w-full bg-white border-2 border-blue-100 pl-9 pr-3 py-2.5 rounded-xl font-bold text-sm focus:border-blue-500 outline-none transition-all" placeholder="e.g. City Bank Transfer, Chq #9981" required />
                            </div>
                        </div>
                    )}
                </div>

                {/* বটম ক্যালকুলেশন সামারি এবং ফুল্লি পেইড টগল */}
                <div className="bg-gray-900 p-8 rounded-[2rem] flex flex-col sm:flex-row justify-between items-center gap-6 shadow-2xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full -mr-16 -mt-16 group-hover:scale-110 transition-transform duration-700"></div>
                    ...
                    <div className="relative z-10 flex flex-col items-center sm:items-end gap-3 border-t sm:border-t-0 sm:border-l border-gray-800 pt-6 sm:pt-0 sm:pl-8">
                        <label className="flex items-center gap-3 cursor-pointer group">
                            <input
                                type="checkbox"
                                name="is_fully_paid"
                                ...
                                onChange={handleChange}
                                className="w-6 h-6 rounded-lg bg-gray-800 border-gray-700 text-green-500 focus:ring-green-500/20"
                            />
                            <span className="text-xs font-black uppercase tracking-widest text-gray-400 group-hover:text-green-400 transition-colors">Fully Repaid</span>
                        </label>
                        <p className="text-xs font-bold text-blue-400">Target Principal: ৳{Number(form.amount || 0).toLocaleString()}</p>
                    </div>
                </div>
            </div>
        </BaseModal>

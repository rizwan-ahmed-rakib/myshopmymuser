...
import { 
  ...
  FaCoins, FaCalendarAlt, FaUserTie
} from "react-icons/fa";
import BASE_URL_of_POS from "../../../posConfig";
import { useUserWithProfile } from "../../../context_or_provider/pos/profile/userWithProfile";
...
const AddEmployeeSalaryAdvanceModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const customSelectStyles = {
        control: (base) => ({ ...base, borderRadius: '1rem', padding: '0.4rem', border: '2px solid #f3f4f6', boxShadow: 'none', '&:hover': { borderColor: '#10b981' } }),
        option: (base, state) => ({ ...base, backgroundColor: state.isSelected ? '#10b981' : state.isFocused ? '#ecfdf5' : 'white', fontWeight: 'bold' }),
            ...
            <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] border border-white/20">
                <div className="px-8 py-6 bg-gray-900 text-white flex justify-between items-center sticky top-0 z-10">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-emerald-600 rounded-2xl flex items-center justify-center text-xl shadow-lg shadow-emerald-600/30"><FaPlusCircle /></div>
                        <div><h2 className="text-xl font-black uppercase tracking-tight">Salary Advance</h2><p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Disburse Advance Payment</p></div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-8 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="flex gap-4 pt-4 border-t-2 border-dashed border-gray-100">
                        ...
                        <button type="submit" disabled={loading || Number(form.amount) <= 0} className="flex-[2] bg-emerald-600 text-white px-8 py-5 rounded-2xl font-black uppercase text-[10px] tracking-[0.3em] shadow-xl shadow-emerald-600/30 active:scale-95 hover:bg-emerald-500 transition-all flex items-center justify-center gap-3 disabled:bg-gray-200 disabled:shadow-none disabled:text-gray-400">
                            {loading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <><FaCheckCircle /> Record Advance</>}
                        </button>
                    </div>
                </form>
            </div>

...
const SuccessModal = ({ isOpen, onClose, employee, type = 'create' }) => {
    ...
    const handlePrint = () => {
        ...
        const content = `
          <html>
            <head>
              <title>Salary Advance Voucher #${employee.id}</title>
              <style>
            ...
            <body>
              <div class="header">
                <div class="company-info"><h1>MY SHOP POS</h1><p>Salary Advance Voucher</p></div>
                <div class="voucher-title"><h2>Advance</h2><p>#${employee.id}</p></div>
              </div>
              <div style="display: flex; justify-content: space-between; margin-bottom: 40px;">
                <div class="info-section"><h3>Employee</h3><p>${employee.user_name}</p></div>
                <div class="info-section" style="text-align: right;"><h3>Date</h3><p>${new Date(employee.request_date).toLocaleDateString()}</p></div>
              </div>
              <table class="table">
                <thead><tr><th>Description</th><th>Method</th><th style="text-align: right;">Amount</th></tr></thead>
                <tbody>
                  ${Number(employee.paid_cash) > 0 ? `<tr><td>Cash Advance</td><td>CASH</td><td style="text-align: right;">৳${parseFloat(employee.paid_cash).toLocaleString()}</td></tr>` : ''}
                  ${Number(employee.paid_mobile) > 0 ? `<tr><td>Mobile Transfer (${employee.mobile_operator})</td><td>MOBILE</td><td style="text-align: right;">৳${parseFloat(employee.paid_mobile).toLocaleString()}</td></tr>` : ''}
                  ${Number(employee.paid_bank) > 0 ? `<tr><td>Bank Transfer (${employee.bank_name})</td><td>BANK</td><td style="text-align: right;">৳${parseFloat(employee.paid_bank).toLocaleString()}</td></tr>` : ''}
                </tbody>
              </table>
              <div class="total-box"><div style="font-size: 10px; text-transform: uppercase; color: #9ca3af;">Total Amount Paid</div><div class="total-amount">৳${parseFloat(employee.amount).toLocaleString()}</div></div>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden border border-white/20">
                ...
                <div className={`${isUpdate ? 'bg-blue-600' : 'bg-emerald-600'} p-10 text-center text-white relative overflow-hidden transition-colors duration-500`}>
                    ...
                    <h2 className="text-2xl font-black uppercase tracking-tight">
                        {isUpdate ? 'Advance Updated' : 'Advance Recorded'}
                    </h2>
                ...
                <div className="p-8 space-y-6">
                    <div className="bg-gray-50 p-6 rounded-[2rem] space-y-4 text-sm border border-gray-100">
                        ...
                        <div className="flex justify-between items-center border-t border-dashed border-gray-200 pt-4">
                            <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Advance Amount</span>

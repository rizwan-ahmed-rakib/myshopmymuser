...
const SuccessModal = ({ isOpen, onClose, employee, type = 'create' }) => {
    ...
    const handlePrint = () => {
        ...
        const content = `
          <html>
            <head>
              <title>Salary Payslip #${employee.id}</title>
              <style>
            ...
            <body>
              <div class="header">
                <div class="company-info"><h1>MY SHOP POS</h1><p>Salary Disbursement Voucher</p></div>
                <div class="voucher-title"><h2>Payslip</h2><p>#${employee.id}</p></div>
              </div>
              <div style="display: flex; justify-content: space-between; margin-bottom: 40px;">
                <div class="info-section"><h3>Employee</h3><p>${employee.user_name}</p></div>
                <div class="info-section" style="text-align: right;"><h3>Salary Period</h3><p>${employee.month}/${employee.year}</p></div>
              </div>
              <table class="table">
                <thead><tr><th>Earnings/Deductions</th><th style="text-align: right;">Amount</th></tr></thead>
                <tbody>
                  <tr><td>Basic Salary</td><td style="text-align: right;">৳${parseFloat(employee.basic_salary).toLocaleString()}</td></tr>
                  <tr><td>Allowances (+)</td><td style="text-align: right;">৳${parseFloat(employee.allowances).toLocaleString()}</td></tr>
                  <tr><td>Deductions (-)</td><td style="text-align: right;">৳${parseFloat(employee.deductions).toLocaleString()}</td></tr>
                </tbody>
              </table>
              <div style="margin-bottom: 30px;">
                ...
                <table class="table" style="margin-bottom: 0;">
                    <tbody>
                      ${Number(employee.paid_cash) > 0 ? `<tr><td>Cash Payout</td><td style="text-align: right;">৳${parseFloat(employee.paid_cash).toLocaleString()}</td></tr>` : ''}
                      ${Number(employee.paid_mobile) > 0 ? `<tr><td>Mobile (${employee.mobile_operator})</td><td style="text-align: right;">৳${parseFloat(employee.paid_mobile).toLocaleString()}</td></tr>` : ''}
                      ${Number(employee.paid_bank) > 0 ? `<tr><td>Bank (${employee.bank_name})</td><td style="text-align: right;">৳${parseFloat(employee.paid_bank).toLocaleString()}</td></tr>` : ''}
                    </tbody>
                </table>
              </div>
              <div class="total-box"><div style="font-size: 10px; text-transform: uppercase; color: #9ca3af;">Net Salary Paid</div><div class="total-amount">৳${parseFloat(employee.net_salary).toLocaleString()}</div></div>
    ...
    return (
        <div className="fixed inset-0 bg-gray-900/90 backdrop-blur-md flex items-center justify-center z-[9999] p-4 animate-in fade-in duration-300">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden border border-white/20">
                ...
                <div className={`${isUpdate ? 'bg-blue-600' : 'bg-indigo-600'} p-10 text-center text-white relative overflow-hidden transition-colors duration-500`}>
                    ...
                    <h2 className="text-2xl font-black uppercase tracking-tight">
                        {isUpdate ? 'Payslip Updated' : 'Salary Disbursed'}
                    </h2>
                ...
                <div className="p-8 space-y-6">
                    <div className="bg-gray-50 p-6 rounded-[2rem] space-y-4 text-sm border border-gray-100">
                        ...
                        <div className="flex justify-between items-center border-t border-dashed border-gray-200 pt-4">
                            <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Net Salary</span>
                            <span className={`font-black text-2xl ${isUpdate ? 'text-blue-600' : 'text-indigo-600'} font-mono`}>৳{Number(employee.net_salary).toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center border-t border-dashed border-gray-200 pt-4">
                            <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Period</span>
                            <span className="font-black text-sm text-gray-900">{new Date(0, employee.month-1).toLocaleString('default', { month: 'long' })} {employee.year}</span>
                        </div>
                    </div>

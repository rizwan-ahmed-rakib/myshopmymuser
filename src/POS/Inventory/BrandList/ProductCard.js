...
//         navigate(`/inventory/brand/details/${product.id}`);
//     };
...
//         try {
//             await posBrandAPI.delete(product.id); // Corrected API object
...
const ProductCard = ({ product, onEdit, onDelete }) => {
  ...
  const handleNameClick = () => navigate(`/inventory/brand/details/${product.id}`);

  const handleEdit = (p) => { setSelectedProduct(p); setShowEditModal(true); setShowDropdown(false); };

  const handleDelete = async (p) => {
    if (!window.confirm(`"${p.title}" ডিলিট করবেন?`)) return;
    setLoadingId(p.id);
    try {
      await posBrandAPI.delete(p.id);
      setSuccessMessage(`${p.title} deleted successfully!`);
      setShowSuccess(true);
  ...
  return (
    <>
      <div className="bg-white rounded-xl border border-slate-200 hover:border-indigo-300 hover:shadow-md transition-all duration-200 overflow-hidden group">
          ...
          <div className="absolute top-2.5 right-2.5" ref={dropdownRef}>
            ...
            {showDropdown && (
              <div className="absolute right-0 mt-1 w-44 bg-white border border-slate-200 rounded-xl shadow-lg py-1.5 z-10">
                <button onClick={() => { handleEdit(product); setShowDropdown(false); }} className="flex items-center gap-2.5 w-full px-3.5 py-2 text-[12px] text-slate-700 hover:bg-slate-50">
                ...
                <button onClick={() => { handleDelete(product); setShowDropdown(false); }} disabled={loadingId === product.id} className="flex items-center gap-2.5 w-full px-3.5 py-2 text-[12px] text-rose-600 hover:bg-rose-50">
        ...
        <div className="p-3.5">
          <h3
            onClick={handleNameClick}
            className="text-[14px] font-semibold text-slate-800 hover:text-indigo-600 cursor-pointer truncate transition-colors leading-tight mb-1"
          >
            {product.title}
          </h3>

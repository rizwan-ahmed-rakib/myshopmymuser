...
const ProductList = ({ products, onUpdate }) => {
  ...
  const handleViewDetails = (product) => navigate(`/inventory/product/brand/${product.id}`);

  const handleEdit = (product) => { setSelectedProduct(product); setShowEditModal(true); };

  const handleDelete = async (product) => {
    if (!window.confirm(`"${product.title}" ডিলিট করবেন?`)) return;
    setLoadingId(product.id);
    try {
      await posBrandAPI.delete(product.id);
        ...
        <div className="divide-y divide-slate-100">
              ...
              <div className="grid grid-cols-12 gap-4 items-center">
                ...
                <div className="col-span-5 flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg overflow-hidden border border-slate-200 shrink-0 bg-slate-100">
                    <img
                      src={product.image || "https://via.placeholder.com/150?text=B"}
                      alt={product.title}
                      className="w-full h-full object-cover"
                      onError={(e) => { e.target.src = "https://via.placeholder.com/150?text=B"; }}
                    />
                  </div>
                  <div className="min-w-0">
                    <p
                      onClick={() => handleViewDetails(product)}
                      className="text-[13px] font-medium text-slate-800 hover:text-indigo-600 cursor-pointer truncate transition-colors"
                    >
                      {product.title}
                    </p>
                ...
                {/* ID */}
                <div className="col-span-3">
                  <span className="text-[12px] text-slate-500 font-mono">#{product.id}</span>
                </div>
          ...
          {(!products || products.length === 0) && (
            <div className="py-16 text-center">
              ...
              <p className="text-[13px] font-medium text-slate-700">No brands found</p>
              <p className="text-[12px] text-slate-400 mt-1">Add your first brand to get started</p>
            </div>
          )}
        </div>
      </div>

...
import { Tag, Camera, Image as ImageIcon, Settings } from 'lucide-react';
import { posBrandAPI } from "../../../context_or_provider/pos/brands/brandAPI";
import BaseModal from "../../components/BaseModal";
import { useForm } from "../../../hooks/profile";

/**
 * UpdateBrandModal - Refactored to use BaseModal and match Backbone pattern.
 */
const UpdateBrandModal = ({ isOpen, onClose, onSuccess, productData }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await posBrandAPI.update(productData.id, dataToSend);
            onSuccess?.(res.data);
            onClose();
        } catch (err) {
            console.error(err);
    ...
    return (
        <BaseModal
            ...
            onClose={onClose}
            title="Update Brand"
            subtitle={`Ref: #BRD-${productData?.id}`}
            size="md"
            icon={<Settings />}
            showFooter={true}
        ...
        >
            <div className="space-y-8">
                ...
                <div className="bg-gray-50 p-8 rounded-[2rem] border-2 border-dashed border-gray-200 flex flex-col items-center text-center shadow-inner relative group hover:border-blue-400 transition-colors">
                    ...
                    <div className="mt-4">
                        <h4 className="text-sm font-black text-gray-800 uppercase tracking-tight">{form.title || "Brand Logo"}</h4>

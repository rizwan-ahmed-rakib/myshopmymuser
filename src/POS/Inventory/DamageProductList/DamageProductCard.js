...
// const DamageProductCard = ({product, onEdit, onDelete}) => {
...
//         try {
//             await posDamageProductAPI.delete(product.id); // Corrected API object
...
// export default DamageProductCard;
...
const DamageProductCard = ({ product, onEdit, onDelete }) => {
  ...
  const handleNameClick = () => navigate(`/inventory/damage-product/details/${product.id}`);
  ...
  return (
    <>
      <div className="bg-white rounded-xl border border-slate-200 hover:border-rose-300 hover:shadow-md transition-all duration-200 overflow-hidden group">
        ...
        {/* Info */}
        <div className="p-3.5">
          <h3 onClick={handleNameClick} className="text-[14px] font-semibold text-slate-800 hover:text-rose-500 cursor-pointer truncate transition-colors leading-tight mb-1">
            {product.name}
          </h3>
          <p className="text-[11px] text-slate-400 mb-2">Code: {product.product_code}</p>
          <div className="flex items-center justify-between">
            <span className="text-[13px] font-semibold text-slate-800">{formatPrice(product.selling_price)}</span>
            <span className="text-[11px] text-slate-400 line-through">{formatPrice(product.purchase_price)}</span>
          </div>
          <div className="mt-1.5 text-[10px] text-slate-400">
            Cat: {product.category} · Brand: {product.brand}
          </div>
        </div>
      </div>

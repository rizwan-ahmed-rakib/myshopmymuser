...
const ExpiredProducts = () => {
  ...
  const expiredCount    = instances.filter(i => i.expiry_status === 'expired').length;
  const nearExpiryCount = instances.filter(i => i.expiry_status === 'near_expiry').length;

  const getStatusStyle = (status) => {
    if (status === 'expired')    return 'bg-rose-50 text-rose-600 border border-rose-100';
    if (status === 'near_expiry') return 'bg-amber-50 text-amber-600 border border-amber-100';
    return 'bg-slate-100 text-slate-500';
  };

  const selectCls = "w-full px-3 py-2 text-[12px] border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-400 bg-white text-slate-700 disabled:opacity-40";
    ...
    <div className="p-4 md:p-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[16px] font-semibold text-slate-800">Expired & Near Expiry Products</h1>
            <p className="text-[12px] text-slate-400">Stock items that are expired or expiring soon</p>
          </div>
        </div>
        <button onClick={fetchData}
          className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white text-[12px] font-medium rounded-lg transition-colors">
      ...
      <div className="flex gap-3 mb-5">
        <div className="flex items-center gap-2 bg-rose-50 border border-rose-100 rounded-xl px-4 py-2.5">
          ...
          <span className="text-[12px] font-semibold text-rose-600">{expiredCount} Expired</span>
        </div>
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-xl px-4 py-2.5">
          <div className="w-2 h-2 rounded-full bg-amber-400" />
          <span className="text-[12px] font-semibold text-amber-600">{nearExpiryCount} Near Expiry</span>
        </div>
        <div className="flex items-center gap-2 bg-slate-100 rounded-xl px-4 py-2.5">
          <span className="text-[12px] font-semibold text-slate-600">{filteredInstances.length} Showing</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 mb-5">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <div className="relative">
            ...
            <input
              type="text" placeholder="Product, Serial, Batch..."
              className="w-full pl-8 pr-3 py-2 text-[12px] border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-400 text-slate-700 placeholder-slate-400"
              value={filters.search}
              onChange={(e) => setFilters({ ...filters, search: e.target.value })}
            />
          </div>
          ...
          <select className={selectCls} value={filters.sub_category} onChange={(e) => setFilters({ ...filters, sub_category: e.target.value })} disabled={filters.category === 'all'}>
            <option value="all">All Sub-Categories</option>
            {availableSubCategories.map(sub => <option key={sub.id} value={sub.id}>{sub.title}</option>)}
          </select>

          <select className={selectCls} value={filters.brand} onChange={(e) => setFilters({ ...filters, brand: e.target.value })}>
            <option value="all">All Brands</option>
            {brands.map(b => <option key={b.id} value={b.id}>{b.name || b.title}</option>)}
          </select>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-[13px] text-slate-500">Loading products...</p>
          </div>
        ) : filteredInstances.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  {['Product', 'Batch & Serial', 'Dates', 'Status', 'Actions'].map((h, i) => (
                    <th key={h} className={`px-5 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wide ${i === 4 ? 'text-right' : ''}`}>
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {filteredInstances.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <p className="text-[13px] font-semibold text-slate-800">{item.product_name || item.product?.name}</p>
                      <p className="text-[10px] text-slate-400 mt-0.5">Code: {item.product?.product_code}</p>
                    </td>

                    {/* Batch & SN */}
                    <td className="px-5 py-3.5">
                      <p className="text-[12px] text-slate-600">Batch: <span className="font-medium">{item.batch_no || 'N/A'}</span></p>
                      <p className="text-[11px] text-slate-400 font-mono mt-0.5">SN: {item.unique_serial}</p>
                    </td>

                    {/* Dates */}
                    <td className="px-5 py-3.5">
                      <p className="text-[11px] text-slate-500">Mfg: {item.manufacturing_date || 'N/A'}</p>
                      <p className="text-[11px] font-semibold text-rose-500 mt-0.5">Exp: {item.expiry_date || 'N/A'}</p>
                    </td>

                    {/* Status */}
                    <td className="px-5 py-3.5">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold ${getStatusStyle(item.expiry_status)}`}>
                        {item.expiry_status?.replace('_', ' ').toUpperCase()}
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <button className="w-7 h-7 flex items-center justify-center ml-auto text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-[14px] font-medium text-slate-700">No products found</p>

...
//         try {
//             await posProductAPI.delete(product.id); // Corrected API object
...
const ProductCard = ({ product, onEdit, onDelete }) => {
  ...
  const handleNameClick = () => navigate(`/inventory/product/details/${product.id}`);
  ...
  const stockBadge = isOutOfStock
    ? { label: "Out of Stock", cls: "bg-slate-500 text-white" }
    : isCritical
    ? { label: "Critical", cls: "bg-rose-500 text-white animate-pulse" }
    : isLow
    ? { label: "Low Stock", cls: "bg-amber-500 text-white" }
    : { label: `In Stock (${product.stock})`, cls: "bg-emerald-500 text-white" };
    ...
    <>
      <div className="bg-white rounded-xl border border-slate-200 hover:border-indigo-300 hover:shadow-md transition-all duration-200 overflow-hidden group">
        ...
        <div className="relative h-44 bg-slate-50 overflow-hidden">
          ...
          />
          <span className={`absolute top-2.5 left-2.5 text-[10px] font-semibold px-2 py-0.5 rounded-full ${stockBadge.cls}`}>
            {stockBadge.label}
          </span>
        ...
        <div className="p-3.5">
          ...
          <h3 onClick={handleNameClick} className="text-[14px] font-semibold text-slate-800 hover:text-indigo-600 cursor-pointer truncate transition-colors leading-tight mb-2">
            {product.name}
          </h3>
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[14px] font-bold text-slate-800">৳{product.selling_price}</span>
            <span className="text-[11px] text-slate-400 line-through">৳{product.purchase_price}</span>
          </div>
          <p className="text-[10px] text-slate-400">Cat: {product.category} · Brand: {product.brand}</p>
        </div>
      </div>

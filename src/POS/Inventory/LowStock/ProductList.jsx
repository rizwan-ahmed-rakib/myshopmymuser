...
const ProductList = ({ products, onUpdate }) => {
  ...
  const handleViewDetails = (p) => navigate(`/inventory/product/details/${p.id}`);
  ...
  const handleDelete = async (p) => {
    ...
    setLoadingId(p.id);
    try {
      await posProductAPI.delete(p.id);
  ...
  const getStockStatus = (product) => {
    ...
    const limit = Number(product.alarm_when_stock_is_lessthanOrEqualto);
    if (stock === 0) return { label: "Out of Stock", cls: "bg-slate-100 text-slate-600 border-slate-200", textCls: "text-slate-500" };
    if (stock <= limit * 0.2) return { label: "Critical", cls: "bg-rose-50 text-rose-700 border-rose-200 animate-pulse", textCls: "text-rose-600 font-bold" };
    if (stock <= limit) return { label: "Low Stock", cls: "bg-amber-50 text-amber-700 border-amber-200", textCls: "text-amber-600 font-semibold" };
    return { label: "In Stock", cls: "bg-emerald-50 text-emerald-700 border-emerald-200", textCls: "text-emerald-600" };
  };
        ...
        <div className="divide-y divide-slate-100">
                ...
                <div className="grid grid-cols-12 gap-4 items-center">
                  ...
                  <div className="col-span-4 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p onClick={() => handleViewDetails(product)} className="text-[13px] font-medium text-slate-800 hover:text-indigo-600 cursor-pointer truncate transition-colors">
                        {product.name}
                      </p>
                  ...
                  {/* Price */}
                  <div className="col-span-2">
                    <p className="text-[12px] font-medium text-slate-700">৳{product.selling_price}</p>
                    <p className="text-[10px] text-slate-400 line-through">৳{product.purchase_price}</p>
                  </div>

                  {/* Stock with level indicator */}
                  <div className="col-span-2">
                    <p className={`text-[13px] font-bold ${stockStatus.textCls}`}>
                      {product.stock} <span className="text-[10px] font-normal text-slate-400">units</span>
                    </p>
                    <span className={`inline-flex text-[9px] font-semibold px-1.5 py-0.5 rounded-full border mt-0.5 ${stockStatus.cls}`}>
                      {stockStatus.label}
                    </span>
                    <p className="text-[9px] text-slate-400 mt-0.5">Limit: {product.alarm_when_stock_is_lessthanOrEqualto}</p>
                  </div>
                  ...
                  <div className="col-span-2 flex items-center justify-end gap-1">
                    ...
                    <button onClick={() => handleEdit(product)} title="Edit" disabled={loadingId === product.id} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">
                      <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {(!products || products.length === 0) && (
            <div className="py-16 text-center">
              ...
              <p className="text-[12px] text-slate-400 mt-1">Add your first product to get started</p>
            </div>
          )}
        </div>
      </div>

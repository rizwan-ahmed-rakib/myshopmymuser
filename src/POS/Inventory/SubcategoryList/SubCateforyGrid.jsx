...
import { posSubCategoryAPI } from "../../../context_or_provider/pos/subcategories/subCategoryApi";
import { useSubPosCategory } from "../../../context_or_provider/pos/subcategories/SubCategoryProvider";
import { GitCommit, ArrowUpDown } from 'lucide-react';
...
const SubCateforyGrid = ({
    ...
    const { setPosSubCategories } = useSubPosCategory();
    const [successData, setSuccessData] = useState(null);
    ...
    useEffect(() => {
        if (setFilterConfig) {
            setFilterConfig({
                searchPlaceholder: "Search sub category by title...",
                filtersConfig: [
    ...
    const calculateStats = useCallback((data) => {
        return [
            { title: 'Total Sub Categories', count: data.length, bgColor: 'bg-orange-600', icon: <GitCommit size={24} /> }
        ];
    }, []);
    ...
    const {
        ...
        rawData: posSubCategories,
        loading,
        refresh
    } = useModuleData({
        apiFetch: posSubCategoryAPI.getAll,
        searchQuery,
        filters,
        searchFields: ['title'],
        onStatsLoaded,
    ...
    useEffect(() => {
        if (posSubCategories) setPosSubCategories(posSubCategories);
    }, [posSubCategories, setPosSubCategories]);

    const handleProductAdded = (newSub) => {
        setIsAddOpen(false);
        setSuccessData(newSub);
        refresh();
    };
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center py-20 w-full">
            <LoadingSpinner size="lg" />
            <p className="mt-4 text-gray-500 text-sm">Loading sub category records...</p>
        </div>
    );
            ...
            <div className="p-1">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 px-1">
                    <h2 className="text-sm font-bold text-gray-700 uppercase tracking-tight flex items-center gap-2">
                        ...
                        {viewType === "grid" ? "Sub Category Grid" : "Sub Category List"}
                    </h2>
                    <div className="text-[11px] font-medium text-gray-400 bg-gray-50 px-2 py-1 rounded-md border border-gray-100">
                        SHOWING {filteredSubCategories.length} OF {posSubCategories?.length || 0} RECORDS
                    </div>
                </div>
                ...
                {viewType === "grid" ? (
                    ...
                    <SubcategoryList products={filteredSubCategories} onUpdate={handleProductUpdated} />
                )}

                {filteredSubCategories.length === 0 && (
                    <div className="text-center py-16 border border-dashed border-gray-200 rounded-xl bg-gray-50/30 mt-4">
                        <GitCommit className="w-8 h-8 text-gray-300 mx-auto mb-4" />
                        <h3 className="text-base font-bold text-gray-800 mb-1">No sub categories found</h3>
                        <button
                            onClick={() => setIsAddOpen(true)}
                            className="px-6 py-2 bg-orange-600 text-white rounded-lg text-xs font-bold mt-4 shadow-md active:scale-95"
                        >
                            Add Sub Category
                        </button>
                    </div>

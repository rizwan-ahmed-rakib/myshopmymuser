...
/**
 * AddUnitModal - Refactored to use BaseModal, useForm, and Backbone branding.
 */
const AddUnitModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const {
        ...
        {
            title: (v) => !v ? "Unit title is required" : null,
        }
    );
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await posUnitAPI.create(form);
            onSuccess?.(res.data);
            handleClose();
        } catch (err) {
            console.error(err);
            const errorMsg = err.response?.data ? JSON.stringify(err.response.data) : "Failed to add unit";
            alert(errorMsg);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <BaseModal
            ...
            subtitle="Define how you count your stock (pcs, kg, etc.)"
            size="md"
            icon={<Scale />}
            showFooter={true}
            onSubmit={handleSubmit}
            submitText="Save Unit"
            isLoading={loading}
        >
            <div className="space-y-8">
                ...
                <div className="bg-gray-50 p-8 rounded-[2rem] border-2 border-dashed border-gray-200 flex flex-col items-center text-center shadow-inner relative group hover:border-blue-400 transition-colors">
                    ...
                    <div className="mt-4">
                        <h4 className="text-sm font-black text-gray-800 uppercase tracking-tight">Unit Icon</h4>
                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Optional Visual Aid</p>
                    </div>
                </div>

                {/* Title Input */}
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Unit Title *</label>
                    <div className="relative">
                        ...
                        <input 
                            ...
                            placeholder="e.g. Piece (pcs), Kilogram (kg), Pack, etc." 
                        />
                    </div>

...
const PurchaseReturnSuccessModal = ({ isOpen, onClose, purchase, title, successMessage }) => {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-scaleIn">
        ...
        <div className="bg-green-500 p-8 text-center text-white">
            ...
            <p className="opacity-80 font-medium">Invoice #{purchase.purchase_invoice_no || purchase.invoice_no}</p>
        </div>
        ...
        <div className="p-6">
            ...
            <div ref={printRef} className="bg-gray-50 rounded-2xl p-5 border-2 border-dashed border-gray-200">
                <div className="header">
                    ...
                    <p style={{fontSize: '10px', margin: '5px 0'}}>Original Invoice: #{purchase.purchase_invoice_no || purchase.invoice_no}</p>
                </div>

                <table className="info-table">
                    <tbody>
                        <tr><td><strong>Date:</strong></td><td align="right">{new Date(purchase.created_at).toLocaleString()}</td></tr>
                        <tr><td><strong>Supplier:</strong></td><td align="right">{purchase.supplier_name || "N/A"}</td></tr>
                    ...
                    <tbody>
                        {purchase.items?.map((item) => (
                            <tr key={item.id}>
                                ...
                                <td align="center">{item.purchase_return_quantity || item.quantity}</td>
                ...
                <div className="totals">
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-200">
                        <p style={{fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase', color: '#666', marginBottom: '5px'}}>Payment Breakdown</p>
                        {Number(purchase.paid_cash) > 0 && <div className="total-row"><span>Cash Received</span><span>৳{purchase.paid_cash}</span></div>}
                        ...
                        {Number(purchase.paid_bank) > 0 && <div className="total-row"><span>Bank Refund</span><span>৳{purchase.paid_bank}</span></div>}
                    </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-8">
                ...
                <button
                    onClick={onClose}
                    className="px-4 py-3 bg-gray-100 text-gray-700 rounded-xl font-bold hover:bg-gray-200 transition-all border border-gray-200 active:scale-95"
                >
                    Close
                </button>
            </div>
...
export default PurchaseReturnSuccessModal;
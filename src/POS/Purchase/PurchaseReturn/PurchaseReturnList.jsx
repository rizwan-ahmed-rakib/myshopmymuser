...
/**
 * PurchaseReturnList - Refactored to use BackboneTable and StatusBadge.
 * Standardized list view for Purchase Return records.
 */
const PurchaseReturnList = ({ purchaseReturns, onEdit, onDelete }) => {
    ...
    const handleDelete = async (item) => {
        if (!window.confirm(`Are you sure you want to delete return for invoice #${item.purchase_invoice_no}?`)) return;
            ...
            render: (item) => (
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col min-w-0">
                        <span className="font-bold text-gray-900 truncate">#{item.purchase_invoice_no}</span>
        ...
        },
        {
            ...
            render: (item) => (
                <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">Net: ৳{parseFloat(item.net_return_amount).toLocaleString()}</span>
                </div>
            )
        },
        {

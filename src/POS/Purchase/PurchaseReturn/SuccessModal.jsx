 ...
 * Tailored for Purchase Return success with unified printing engine.
 */
const SuccessModal = ({ isOpen, onClose, purchase, title, successMessage }) => {
    ...
    const receivedBack = parseFloat(purchase.paid_amount || 0);
    const pendingRefund = parseFloat(purchase.due_amount || 0);

    const handlePrint = () => {
        if (!purchase) return;
        const tableContent = getPurchaseReturnPrintLayout(purchase);
        const fullHTML = getBrandedVoucher("Purchase Return", tableContent, purchase.id, "#dc2626");
        ...
        >
            <div className="text-center space-y-6 py-2">
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest">{successMessage || `Return Record #${purchase.id} Created`}</p>
                </div>
                ...
                <div className="bg-gray-50 rounded-2xl p-5 space-y-4 border border-gray-100 text-left">
                    ...
                    <div className="pt-3 border-t border-dashed border-gray-200">
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-[10px] font-black text-emerald-500 uppercase tracking-wider">Received Back</span>
                            <span className="font-black text-emerald-600 text-base">৳{receivedBack.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-[10px] font-black text-amber-500 uppercase tracking-wider">Pending Refund</span>

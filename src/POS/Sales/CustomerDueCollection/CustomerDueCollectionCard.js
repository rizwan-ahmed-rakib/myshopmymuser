...
const CustomerDueCollectionCard = ({ item, onEdit, onDelete }) => {
    ...
    const handleViewDetails = () => {
        navigate(`/sales/due-collection/details/${item.id}`);
    };

    const handleDelete = async () => {
        if (!window.confirm(`Delete collection record #${item.invoice_no}?`)) return;
    ...
    return (
        ...
        >
            <div className="p-5">
                ...
                <h3 className="font-bold text-gray-900 text-lg mb-1 truncate">{item.customer_name}</h3>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-4">
                    {item.sale_invoice_no ? `Sale: #${item.sale_invoice_no}` : "General Due Collection"}
                </p>

                <div className="bg-gray-50 rounded-xl p-4 flex justify-between items-center shadow-inner">
                    <div>
                        <p className="text-[9px] font-black text-gray-400 uppercase">Collected</p>
                        <p className="text-xl font-black text-blue-600 font-mono">৳{parseFloat(item.amount).toLocaleString()}</p>
                    </div>
...
export default CustomerDueCollectionCard;

...
const CustomerDueCollectionDetails = () => {
  ...
  const handlePrint = () => {
    ...
    const content = `
      <html>
        <head>
          <title>Collection Voucher #${item.invoice_no}</title>
          <style>
        ...
        <body>
          <div class="header">
            <div class="company-info">
              <h1>MY SHOP POS</h1>
              <p style="font-size: 12px; color: #6b7280; font-weight: 600;">Customer Due Collection Voucher</p>
            </div>
          ...
          <div style="display: flex; justify-content: space-between; margin-bottom: 40px;">
            <div class="info-section">
              ...
              <p style="font-size: 12px; color: #6b7280;">${item.customer_phone || ''}</p>
            </div>
          ...
          <table class="table">
            ...
            <tbody>
              ${Number(item.paid_cash) > 0 ? `<tr><td>Cash Received</td><td>CASH</td><td style="text-align: right;">৳${parseFloat(item.paid_cash).toLocaleString()}</td></tr>` : ''}
  ...
  if (!item) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-8">
      <div className="bg-white p-10 rounded-3xl shadow-xl text-center max-w-md">
        ...
        <p className="text-gray-500 font-medium mb-8">The collection record you are looking for might have been deleted or moved.</p>
  ...
  const InfoCard = ({ icon, title, value, color = "text-gray-900", subValue }) => (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-5 hover:shadow-md transition-shadow">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl ${color.replace('text-', 'bg-').replace('600', '50').replace('700', '50')} ${color}`}>
          {icon}
        </div>
        <div>
          ...
          <div className="bg-gray-900 text-white rounded-[2.5rem] p-8 md:p-12 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="bg-emerald-600 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-[0.2em]">Collection Voucher</span>
                    <span className="bg-white/10 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-[0.2em]">{item.status || "Received"}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-6 mt-4 opacity-60">
                    <div className="flex items-center gap-2 font-bold text-xs">
                      <FaRegCalendarAlt className="text-emerald-400"/> {new Date(item.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}
                    </div>
                    <div className="flex items-center gap-2 font-bold text-xs">
                      <FaHashtag className="text-emerald-400"/> Sale Invoice: {item.sale_invoice_no || "N/A"}
                    </div>
                  </div>
              </div>
              <div className="mt-8 md:mt-0 text-right relative z-10">
                  <p className="text-[10px] font-black uppercase text-gray-400 tracking-[0.3em] mb-2">Total Received</p>
                  <p className="text-5xl md:text-6xl font-black text-white leading-none">
                    <span className="text-emerald-500 text-3xl mr-1">৳</span>{parseFloat(item.amount).toLocaleString()}
                  </p>
              </div>
          </div>
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <InfoCard 
                    ...
                    subValue={item.customer_phone || "Contact not available"}
                  />
                  <InfoCard 
                ...
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
                    ...
                    <div className="grid gap-4">
                        {Number(item.paid_cash) > 0 && (
                          <div className="flex items-center justify-between p-5 bg-emerald-50/30 rounded-2xl border border-emerald-50">
                            <div className="flex items-center gap-4">
                              <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center"><FaMoneyBillWave/></div>
                              <div>
                                <p className="font-black text-sm text-gray-900">Cash Received</p>
                                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Hand-to-hand cash</p>
                              </div>
                            </div>
                            <span className="font-black text-lg text-emerald-700 font-mono">৳{parseFloat(item.paid_cash).toLocaleString()}</span>
                          </div>
                        )}
                          ...
                          <div className="flex items-center justify-between p-5 bg-blue-50/30 rounded-2xl border border-blue-50">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">A/C: {item.bank_name || 'Direct Deposit'}</p>
                              </div>
                            </div>
                    ...
                    <div className="mt-8 pt-8 border-t-2 border-dashed border-gray-100 flex justify-between items-center">
                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Total Collected</span>
                ...
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
                    ...
                    <div className="bg-amber-50/50 p-6 rounded-2xl border border-amber-50">
                      <p className="text-sm font-medium text-gray-700 italic leading-relaxed">
                        {item.note || "No additional remarks provided for this collection."}
                      </p>
                    </div>
                </div>
                ...
                <div className="bg-gray-900 p-8 rounded-[2rem] shadow-xl text-white overflow-hidden relative">
                    ...
                    <div className="space-y-6">
                        <div className="flex gap-4">
                            ...
                            <div>
                              <p className="text-[10px] font-black uppercase text-emerald-400">Collected At</p>
                        ...
                        <div className="flex gap-4">
                            ...
                            <div>
                              <p className="text-[10px] font-black uppercase text-blue-400">Last Updated</p>
                    ...
                    <div className="mt-8 flex items-center gap-2 text-xs font-black text-emerald-400 uppercase tracking-widest">

...
/**
 * AddCustomerDueCollectionModal - Refactored to use BaseModal and standardized backbone layout.
 */
const AddCustomerDueCollectionModal = ({ isOpen, onClose, onSuccess, customers = [] }) => {
  ...
  useEffect(() => {
    if (form.customer && isOpen) {
      const fetchPurchases = async () => {
        ...
        try {
          ...
          const filtered = purchasesData.filter(p => {
             const custId = typeof p.customer === 'object' ? p.customer.id : p.customer;
             return Number(custId) === Number(form.customer) && Number(p.due_amount) > 0;
          });
      ...
      setCustomerPurchases([]); setForm(prev => ({ ...prev, purchase: "" }));
    }
  ...
  const purchaseOptions = useMemo(() =>
    customerPurchases.map(p => ({ value: p.id, label: `#${p.invoice_no} (Due: ৳${p.due_amount})`, due: p.due_amount })),
    [customerPurchases]
  );
  ...
  const loadInvoiceOptions = async (inputValue) => {
    ...
    try {
      ...
      return purchasesData.filter(p => Number(p.due_amount) > 0).map(p => ({
          value: p.id, label: `#${p.invoice_no} - ${p.customer_name} (Due: ৳${p.due_amount})`,
          customer: typeof p.customer === 'object' ? p.customer.id : p.customer, due: p.due_amount
        }));
  ...
  const handleGlobalInvoiceSelect = (option) => {
    if (option) {
      setForm(prev => ({ ...prev, customer: option.customer, purchase: option.value, paid_cash: parseFloat(option.due), paid_mobile: 0, paid_bank: 0 }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post(`/api/sale/due-collections/`, { ...form, amount: totalAmount, sale: form.purchase || null });
  ...
  return (
    <BaseModal
      isOpen={isOpen}
      onClose={handleClose}
      title="Record Customer Due Collection"
      size="md"
      icon={<PlusCircle className="text-white" />}
      showFooter={!showSuccess}
      onSubmit={handleSubmit}
      submitText="Confirm Collection"
      isLoading={loading}
    >
      {showSuccess && invoiceData ? (
          <div className="text-center space-y-6 py-2">
              ...
              <div className="space-y-1">
                  <h3 className="text-xl font-black text-gray-800 uppercase tracking-tighter">Payment Received</h3>
              ...
              <div className="bg-gray-50 rounded-2xl p-6 space-y-4 border border-gray-100 text-left">
                  <div className="flex justify-between items-center">
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-wider">Collected Amount</span>
                  ...
                  <div className="pt-4 border-t border-dashed border-gray-200">
                      <div className="flex justify-between items-center text-xs">
                          ...
                          <span className="font-black text-gray-800">{invoiceData.customer_name}</span>
                      </div>
                  </div>
              </div>
          ...
          <div className="space-y-6">
              ...
              <div className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100 space-y-3">
                  ...
                  />
                  <p className="text-[9px] text-gray-400 font-bold italic uppercase tracking-tighter">* Search across all customers with due</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Select Customer *</label>
                      <Select options={customerOptions} onChange={(opt) => setForm(prev => ({ ...prev, customer: opt ? opt.value : "", purchase: "" }))} placeholder="Search name..." isClearable styles={customSelectStyles} value={customerOptions.find(o => o.value === Number(form.customer))} menuPortalTarget={document.body} />
                  </div>
                  <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Linked Sale</label>
                      <Select options={purchaseOptions} onChange={(opt) => setForm(prev => ({ ...prev, purchase: opt ? opt.value : "", paid_cash: opt ? parseFloat(opt.due) : prev.paid_cash }))} placeholder="Select due sale..." isClearable styles={customSelectStyles} isDisabled={!form.customer || purchasesLoading} value={purchaseOptions.find(o => o.value === Number(form.purchase))} menuPortalTarget={document.body} />
                  </div>
              </div>
              ...
              {selectedCustomer && (
                  <div className="bg-blue-600 p-6 rounded-2xl flex justify-between items-center shadow-lg relative overflow-hidden group">
                      ...
                      <div className="relative z-10">
                          <p className="text-[10px] font-black text-blue-200 uppercase tracking-widest mb-1">Customer Outstanding</p>
                          <p className="text-3xl font-black text-white leading-none">৳{Number(selectedCustomer.due_amount).toLocaleString()}</p>
                      </div>
              ...
              <div className="space-y-4">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                      <Banknote size={12} className="text-brand-primary" /> Collection Breakdown
                  </label>
              ...
              <div className="bg-gray-900 p-6 rounded-[2rem] flex justify-between items-center shadow-xl">
                  <div>
                      <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Total Collection</p>
                      <p className="text-3xl font-black text-emerald-400 font-mono">৳{totalAmount.toLocaleString()}</p>
                  </div>
                  {selectedCustomer && (
                      <div className="text-right">
                          <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">New Balance</p>
                          <p className="text-xl font-black text-rose-500 font-mono">৳{(Number(selectedCustomer.due_amount) - totalAmount).toLocaleString()}</p>
                      </div>
                  )}
              </div>

...
/**
 * AddCustomerModal - Refactored to use BaseModal and standardized backbone layout.
 */
const AddCustomerModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post(`/api/sale/customers/`, formData, {
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col items-center py-2">
            ...
            <p className="mt-3 text-[10px] font-black text-gray-400 uppercase tracking-widest">Customer Photo</p>
        </div>
        ...
        <div className="grid gap-5">
            <div className="space-y-1.5">
                ...
                <input name="name" value={form.name} onChange={handleChange} placeholder="e.g. John Doe" className="w-full border border-gray-200 p-3 rounded-xl font-bold text-gray-800 focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all" required />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                    ...
                    <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="customer@example.com" className="w-full border border-gray-200 p-3 rounded-xl font-bold text-gray-800 focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all" />
                </div>
            </div>
            ...
            <div className="bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100 space-y-2">
                <label className="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-1.5">
                    <Wallet size={12} /> Opening Balance (Optional)
                </label>
                <input type="number" name="due_amount" value={form.due_amount} onChange={handleChange} placeholder="Positive for Due, Negative for Advance" className="w-full border border-emerald-100 p-3 rounded-xl font-black text-emerald-700 focus:ring-4 focus:ring-emerald-100 focus:border-emerald-300 outline-none transition-all bg-white shadow-inner" />
            </div>
        </div>

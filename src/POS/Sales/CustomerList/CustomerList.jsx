            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="overflow-x-auto">
                    ...
                    <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100">
                        <div
                            className="grid grid-cols-12 gap-4 min-w-[800px]"> {/* min-w added so grid doesn't shrink too much */}
                        ...
                        {employees?.map((employee) => (
                            ...
                            >
                                <div className="grid grid-cols-12 gap-4 items-center min-w-[800px]">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <span
                                                        className={`absolute bottom-0 right-0 block h-3 w-3 rounded-full ring-2 ring-white ${
                                                            employee.due_amount > 0 ? 'bg-red-400' : employee.due_amount < 0 ? 'bg-green-400' : 'bg-gray-300'
                                                        }`}/>
                                                </div>
                                            </div>
                                            <div className="ml-4">
                                                <div className="flex items-center">
                                                    ...
                                                    {employee.user?.is_active === false && (
                                                        <span
                                                            className="ml-2 px-2 py-0.5 text-xs font-medium bg-red-100 text-red-800 rounded-full whitespace-nowrap">
                                                Inactive
                                            </span>
                                                    )}
                                                </div>
                                    ...
                                    {/* Status / Due Amount (Designed as Badge) */}
                                    <div className="col-span-2">
                            <span
                                className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${
                                    employee.due_amount > 0 ? 'bg-red-100 text-red-700' :
                                        employee.due_amount < 0 ? 'bg-green-100 text-green-700' :
                                            'bg-gray-100 text-gray-600'
                                }`}>
                                {employee.due_amount > 0
                                    ? `Receiveable ${employee.due_amount}`
                                    : employee.due_amount < 0
                                        ? `Payable ${Math.abs(employee.due_amount)}`
                                        : 'Clear'
                                }
                            </span>
                                    </div>

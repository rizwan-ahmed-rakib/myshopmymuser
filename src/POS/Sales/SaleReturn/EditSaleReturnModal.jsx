    ...
    return (
        ...
        >
            <div className="space-y-6">
                <div className="p-3 bg-gray-50 rounded-lg flex justify-between items-center border">
                    <div>
                        <p className="font-semibold text-gray-800">Customer: {form.customer_name || 'Walk-in'}</p>
                        <p className="text-sm text-gray-500 font-medium">Original Invoice: #{form.sale_invoice_no}</p>
                    </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <textarea className="input w-full border-gray-300 rounded-lg" rows="1"
                    ...
                    <div>
                        ...
                        <textarea className="input w-full border-gray-300 rounded-lg" rows="1" value={form.note || ""}
                ...
                <div className="border rounded-xl overflow-hidden shadow-sm">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <th className="px-4 py-3 text-center text-xs font-bold text-gray-500 uppercase">Return Qty
                            </th>
                        ...
                        {form.items.map((item, index) => (
                            <tr key={item.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-4 py-3">
                                    <input type="number" className="input w-20 text-center mx-auto block"
                                           value={item.sale_return_quantity}
                                           onChange={e => handleItemChange(index, "sale_return_quantity", e.target.value)}/>
                                    {errors[index] &&
                                        <p className="text-[10px] text-red-500 text-center mt-1">{errors[index]}</p>}
                                </td>
                                <td className="px-4 py-3 text-right text-sm">৳{item.unit_price}</td>
                                <td className="px-4 py-3">
                                    <input type="number"
                                           className="input w-24 text-right mx-auto block text-red-600 font-bold"
                                           value={item.penalty_amount}
                                           onChange={e => handleItemChange(index, "penalty_amount", e.target.value)}/>
                                </td>
                                <td className="px-4 py-3 text-right text-sm font-bold">৳{item.total_price}</td>
                                <td className="px-4 py-3 text-center">
                                    <button onClick={() => handleRemoveItem(index)}
                                            className="text-red-400 hover:text-red-600"><FaTrash size={14}/></button>
                                </td>
                            </tr>
                        ))}
                        </tbody>
                ...
                <div className="grid lg:grid-cols-2 gap-8">
                    <div className="bg-blue-50 p-5 rounded-2xl border border-blue-100 space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div><label className="text-[10px] uppercase font-bold text-blue-700">Cash</label><input
                                type="number" className="w-full border-blue-200 p-2 rounded-lg font-bold"
                                value={paidCash} onChange={(e) => setPaidCash(Number(e.target.value))}/></div>
                            <div><label className="text-[10px] uppercase font-bold text-blue-700">Mobile</label><input
                                type="number" className="w-full border-blue-200 p-2 rounded-lg font-bold"
                                value={paidMobile} onChange={(e) => setPaidMobile(Number(e.target.value))}/></div>
                            <div><label className="text-[10px] uppercase font-bold text-blue-700">Bank</label><input
                                type="number" className="w-full border-blue-200 p-2 rounded-lg font-bold"
                                value={paidBank} onChange={(e) => setPaidBank(Number(e.target.value))}/></div>
                        </div>
                        {paidMobile > 0 && (
                            <div
                                className="grid grid-cols-2 gap-3 p-3 bg-white rounded-lg border border-blue-100 shadow-sm">
                                <div><label className="text-[10px] font-bold text-gray-500">Operator</label><select
                                    className="w-full border-0 p-0 text-sm focus:ring-0" value={mobileOperator}
                                    onChange={(e) => setMobileOperator(e.target.value)}>
                                    <option value="bkash">bKash</option>
                                    <option value="nagad">Nagad</option>
                                </select></div>
                                <div><label className="text-[10px] font-bold text-gray-500">Trx ID</label><input
                                    className="w-full border-0 p-0 text-sm focus:ring-0" value={transactionId}
                        ...
                        {paidBank > 0 && (
                            <div className="p-3 bg-white rounded-lg border border-blue-100 shadow-sm">
                                ...
                                <input className="w-full border-0 p-0 text-sm focus:ring-0" value={bankAccountNo}
                    ...
                    <div className="bg-gray-900 text-white p-6 rounded-2xl space-y-3 shadow-xl">
                        <div className="flex justify-between text-sm opacity-70">
                            <span>Gross Total</span><span>৳{form.total_return_amount}</span></div>
                        <div className="flex justify-between text-sm text-red-400">
                            <span>Item Penalties</span><span>- ৳{form.total_item_penalty}</span></div>
                        <div className="flex justify-between items-center text-sm text-red-400">
                            <span>Global Penalty</span>
                            <input type="number"
                                   className="w-24 bg-gray-800 border-0 rounded text-right p-1 font-mono text-white focus:ring-1 focus:ring-red-500"
                                   value={globalPenalty} onChange={e => setGlobalPenalty(Number(e.target.value))}/>
                        </div>
                        <div className="flex justify-between text-lg pt-2 border-t border-gray-800"><span
                            className="font-bold">Net Return</span><span
                            className="font-mono font-black text-2xl text-green-400">৳{form.net_return_amount}</span>
                        </div>
                        <div className="flex justify-between text-sm pt-2 border-t border-gray-800 opacity-70"><span>Total Paid Back</span><span>৳{(Number(paidCash) + Number(paidMobile) + Number(paidBank)).toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between text-lg text-red-500 font-bold">
                            <span>Balance Due</span><span className="font-mono">৳{form.due_amount}</span></div>
                    </div>
                </div>
            </div>
        </BaseModal>
